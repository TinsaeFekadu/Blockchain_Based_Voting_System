...
class Blockchain {
    constructor() {
        ...
        this.creatNewBlock(1, '0', 'Genesis block');
    }
    ...
    makeNewTransaction(cast, voter, candidate) {
        const transaction = {
            cast: cast,
            voter: voter,
            candidate: candidate,
    ...
    findTransactionsByAddress(address) {
        ...
        this.chain.forEach(block => {
            block.transactions.forEach(transaction => {
                if (transaction.voter === address || transaction.candidate === address) {
        ...
        transactions.forEach(transaction => {
            if (transaction.voter === address) {
                noOfCast -= +transaction.cast;
            } else if (transaction.candidate === address) {
                noOfCast += +transaction.cast;
            }
        });
